defmodule Cachets.Saver do
  ...
  def handle_info {:"ETS-TRANSFER", table_name, _pid, "transfered from worker"}, state do
    receive do
      {:return_table_for_worker, pid} -> :ets.give_away(table_name, pid, "return back worker-table")
                                         {:noreply, state}
    after
      500 -> {:noreply, state}
    end
  ...
  def handle_info {:'ETS-TRANSFER', @common_table, _pid, "transfered from common"}, state do
    receive do
      {:return_table_for_common, pid} -> :ets.give_away(@common_table, pid, "return back common-table")
                                         {:noreply, state}
    after
      500 -> {:noreply, state}
    end
